	...
	if (request.getMethod().equalsIgnoreCase("post")) {
	    ...
	    try {
	    	if (action.equals("finalize")) {
		    	UserRentService.getInstance().closure(Long.parseLong(rentId));
		    	sucesso = "Aluguel concluido com sucesso!";
		    } else if (action.equals("cancel")) {
		    	UserRentService.getInstance().cancel(Long.parseLong(rentId));
...
<html lang="pt-br">
	<head>
	    ...
	    <title>Meus aluguéis | Natureza Viva</title>
		...
		<main class="mt-16 flex-grow flex items-center justify-center w-full px-4">
		    <section class="max-w-6xl w-full h-[750px] rounded-lg overflow-y-auto" role="region" aria-labelledby="list-title">
		        <div class="grid gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4">
		            <% for (RentResponse r : UserRentService.getInstance().findByUser(user.id())) { %>
		                <form action="<%= request.getRequestURI() %>" method="post" class="bg-white rounded-lg shadow p-4 flex flex-col justify-between h-full min-h-[300px]">
		                    <div class="flex-grow h-[220px] max-h-[220px] overflow-y-auto">
		                        ...
		                        <p class="text-muted-foreground mt-1 text-sm font-semibold"><%= rentType.get(r.rentType()) %></p>
		                    </div>
		                    <input type="hidden" name="id" value="<%= r.rentUserId() %>">
		                    <% if (r.rentStatus().equals(3L)) { %>
	                            <button name="action" value="finalize" class="w-full mt-2 bg-green-500 text-white font-semibold py-2 px-4 rounded hover:bg-green-600 focus:outline-none focus:ring-2 focus:ring-green-400 focus:ring-offset-2 text-center" aria-label="Finalizar o aluguel do espaço">
	                                Concluir reserva
	                            </button>
		                    <% } else if (r.rentStatus().equals(2L)) { %>
		                    	<button name="action" value="cancel" class="w-full mt-2 bg-rose-500 text-white font-semibold py-2 px-4 rounded hover:bg-rose-600 focus:outline-none focus:ring-2 focus:ring-rose-400 focus:ring-offset-2 text-center" aria-label="Cancelar aluguel do espaço">
